	...
	private static boolean run(BufferedReader br) throws Exception
	{
		...
		for (int q = 0; q < numQuery; ++q)
		{
			...
			case CMD_NUMBER_OF_CANDIDATE:
				...
				{
					isCorrect = false;
					System.out.println("CMD_NUMBER_OF_CANDIDATE : " + ans + " but " + userAns);
				}
				break;
			case CMD_MAX_AREA:
				M = Integer.parseInt(st.nextToken());
				...
				{
					isCorrect = false;
					System.out.println("CMD_MAX_AREA : " + ans + " but " + userAns);
					for (int i = 0; i < M; i++)
            			System.out.print(mStructure[i] + " ");
            		System.out.println(mSeaLevel + " " + ans);
				}
				break;
			default:
				isCorrect = false;
